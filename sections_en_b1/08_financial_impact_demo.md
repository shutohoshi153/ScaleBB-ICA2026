...
In this chapter, we show the practical meaning of the "repositioning as a scenario generator" from §7. We show it in numbers, as the sensitivity of the BEL (present value of benefits) in a simple projection model. The goal is not a precise valuation of liabilities. Instead, we show the direction and the size of the effect that changes in the long-term improvement rate $L$ have on the present value of benefits. We present this in a form that can be applied directly in an economic-value-based evaluation framework. Japan started to apply the economic-value-based solvency regulation (ESR) on 31 March 2026. Within the limits of a simple model, we made the calculation follow the ESR implementation rules as closely as possible (§8.1).
...
2. Discount rate. For the discount factor $P(t)$, we use a risk-free interest rate curve rebuilt by Smith-Wilson extrapolation. We use the parameters of the "yield curve creation tool" published by the Financial Services Agency (FSA), as of the end of March 2026. These are: the last observed maturity for yen interest rates, 30 years; the ultimate forward rate (UFR: Ultimate Forward Rate), 3.8%; the convergence maturity, 60 years; and the zero-coupon risk-free rates by maturity (13 observed maturities). The curve matches the observed maturities exactly. The instantaneous forward rate at the convergence maturity is within ±1bp of the UFR. We do not apply spread adjustments such as the general bucket.
...
4. Use of the regulatory stress factors. For some scenarios (ESR_M and the level part of ICS_C in §8.2), we use the exact stress factors set by the Pillar 1 regulatory notice (FSA Regulatory Notice No. 74 of 2025). These are: incidence rate +20% for morbidity and disability risk (Articles 59 and 60: product class that pays a lump sum when a health event occurs, policy term over 5 years, Japan), and mortality rate +12.5% for mortality risk (Article 56: Japan).
...
![BEL sensitivity by model point and scenario (change rate vs BASE, %, ordered by entry age)](figures/fig_8_1_bel_sensitivity_bar.png)
...
We discuss the implementation burden in §9.2.
...
*The demonstration in this chapter showed one thing. The explicit output of improvement rates, whose direction was validated in advance (§6–§7), translates into concrete sensitivity numbers on the implementation rules of the first year of ESR application. In the next chapter, §9, we discuss the calibration guidance and the implementation burden for introducing this into practice.*
