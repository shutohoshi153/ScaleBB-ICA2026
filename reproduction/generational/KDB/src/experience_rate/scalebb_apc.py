"""Scale BB APC (Age-Period-Cohort) 拡張の KDB 統合モジュール.

KDB 内蔵の ``_scalebb_core.apc_model`` (``fit_scale_bb_apc`` /
``project_scale_bb_apc``) を直接呼び出し、SQLite DB の
``scalebb_run`` / ``scalebb_improvement`` / ``scalebb_projection`` テーブルに
加えて、``scalebb_cohort_effect`` テーブルへ γ(c) を保存する。
...
公開 API::
    ...
    load_apc_projection_to_db(...)  # scalebb_projection (+ γ 外挿)

設計方針:
    - Phase 1 fit/Phase 2 project を **単一の Python 呼び出し内で完結** させる。
      APC 固有の γ(c) を ``fit_scale_bb_apc`` の戻り値から直接 DB に書き込める。
    - 既存 AP 版 (``scalebb.py``) とテーブルを共有するため、APC 固有情報は
      ``scalebb_run.config_json`` と ``scalebb_cohort_effect`` に分離格納する。
"""
...
def run_apc_fit(
    *,
    ...
    """Scale BB APC Phase 1 fit を実行し parquet/CSV + DB に保存.
    ...
    if not matrices:
        raise RuntimeError("APC fit に使用できる観測行列が存在しません")
...
def run_apc_project(
    *,
    ...
    """APC fit 結果 parquet から Phase 2 project を実行し DB に保存.

    APC の fit 結果には ``alpha`` / ``beta`` / ``gamma`` の分解情報が必要だが、
    出力 CSV には平滑化率・改善率のみ保存されている。そのため本関数は
    **fit を fit_file の数値から再構築して投影する** 簡易戦略を採る:

    1. fit_file から rate_smoothed / improvement_smoothed を復元
    2. ScaleBBConfig (AP 版) で project (long_term_rate ブレンド)
    3. 既存 γ(c) は scalebb_cohort_effect に保存済みなのでそのまま継続利用

    γ(c) の外挿 (投影期間の新規コホート) は、scalebb_cohort_effect に
    追加行として書き込む (is_observed=0)。
    """
...
def load_apc_fit_to_db(
    *,
    ...
    """APC fit 結果を DB に upsert (scalebb_improvement + scalebb_cohort_effect)."""
...
def load_apc_projection_to_db(
    *,
    ...
    """APC projection 結果を scalebb_projection に upsert."""
