...
def resolve_scalebb_preset(
    ...
    *,
    ...
    """``scalebb_presets`` から (disease, sex) 文脈でマージ済み設定を返す.

    マージ優先順位 (後勝ち):
        defaults → diseases[disease] → sex[sex]

    返り値のキーは ``lam_row``, ``lam_col``, ``lam_cohort``, ``long_term_rate``,
    ``convergence_year``, ``horizon_year``, ``age_min``, ``age_max``,
    ``covid_mode``, ``covid_years``, ``covid_weight`` 等。
    """
...
def resolve_generational_preset(config: dict) -> dict:
    """``scalebb_presets.generational`` を返す (未定義なら空 dict)."""
    presets = config.get("scalebb_presets", {}) or {}
    return dict(presets.get("generational", {}) or {})


def connect(db_path: str | Path) -> sqlite3.Connection:
    """SQLite 接続を確立 (外部キー ON / ROW_FACTORY)."""
    path = Path(db_path)
    if not path.is_absolute():
        path = PROJECT_ROOT / path
...
def initialize(db_path: str | Path, drop_existing: bool = False) -> None:
    ...
    Args:
        ...
        drop_existing: True の場合、既存 DB ファイルを削除してから再構築
    """
    target = Path(db_path)
