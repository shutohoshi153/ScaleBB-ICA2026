"""KDB 側の Scale BB 拡張モデル統合モジュール.

KDB 内蔵の ``_scalebb_core`` パッケージ (``model`` / ``apc_model`` /
``panels`` / ``disease`` / ``heatmap``) を直接 in-process 呼び出しし、
fit / projection 結果を KDB の SQLite DB へロードして
``analyze_benchmark`` 同様にクエリ可能にする。
...
書込先は ``scalebb_run`` / ``scalebb_improvement`` / ``scalebb_projection``。
"""
...
# ---------------------------------------------------------------------------
# run_fit / run_projection: _scalebb_core を in-process 呼び出し + DB ロード
# ---------------------------------------------------------------------------
def run_fit(
    *,
    db_path: str | Path,
    ...
    """Scale BB Phase 1 fit を実行し、結果を CSV/parquet + DB に書き出す.
...
def run_projection(
    *,
    ...
    """Scale BB Phase 2 projection を実行し、DB へロードする."""
...
def run_heatmap(
    *,
    ...
    """ヒートマップ/投影折れ線図の PNG を生成して保存先一覧を返す."""
...
# ---------------------------------------------------------------------------
# DB ロード: fit CSV / projection CSV → SQLite
# ---------------------------------------------------------------------------
def load_fit_to_db(
    db_path: str | Path,
    fit_file: str | Path,
    *,
    ...
    """Phase 1 fit 結果 CSV/parquet を ``scalebb_improvement`` にロード."""
    path = Path(fit_file)
    df = _read_frame(path)
    if df.empty:
        return 0
...
def load_projection_to_db(
    ...
    *,
    ...
    """Phase 2 projection 結果を ``scalebb_projection`` にロード."""
...
def list_runs(conn: sqlite3.Connection) -> pd.DataFrame:
    """``scalebb_run`` の一覧を DataFrame で返す (新しい順)."""
    try:
...
def _extract_run_metadata(df: pd.DataFrame, meta: dict) -> dict:
    """DataFrame + meta.json から scalebb_run 書込用の属性を抽出."""
