"""Scale BB (AP) fit / projection の高位 Python API.

研究リポジトリ ``ScaleBB_Research/scripts/research/scale_bb_disease.py`` の
``cmd_fit`` / ``cmd_project`` を Python 関数として切り出したもの。
KDB の ``scalebb.py`` が subprocess を経由せずに直接呼び出す。

公開 API::

    fit_result_to_long(...)        ScaleBBFitResult → tidy long DataFrame
    projection_result_to_long(...) 投影済み Result → tidy long DataFrame
    run_fit_to_file(...)           fit 結果を parquet/CSV + meta.json に保存
    run_project_to_file(...)       project 結果を parquet/CSV + meta.json に保存
"""
...
def fit_result_to_long(
    ...
    *,
    ...
    """``ScaleBBFitResult`` を tidy long DataFrame に展開 (実績区間のみ)."""
    rows: list[dict] = []
...
def projection_result_to_long(
    ...
    *,
    ...
    """投影済み ``ScaleBBFitResult`` を long DataFrame に展開 (全期間)."""
...
def run_fit_to_file(
    *,
    ...
    """Scale BB Phase 1 (fit) を実行し parquet/CSV + meta.json を書き出す.
...
def run_project_to_file(
    *,
    ...
    """Scale BB Phase 2 (project) を実行し parquet/CSV + meta.json を書き出す."""
